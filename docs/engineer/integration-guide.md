...
- **The debrief is the tail of the exit sequence.** There is no separate `debrief:` field: author debrief content (study purpose, dehoaxing, any "may we use your data?" withdrawal choice) as the final steps of `exitSequence`. The host renders the whole exit sequence **before** its QC survey and completion code, so the debrief stays gated behind the code — a participant can't collect the completion code and leave before it. Placing the QC survey after the exit sequence also lets it measure the participant's post-debrief exit state.
