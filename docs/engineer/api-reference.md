...
**`Select` picker.** The options list is opted into the customizable select (`appearance: base-select`, #627). On engines that have it — Chrome and Edge 135+, Safari 27 — the picker is a top-layer popover in the page: anchored under the trigger, at least as wide as it, flipping above when there is no room below, and painted with the same tokens as the rest of the control (`--stagebook-surface`, `--stagebook-border`, `--stagebook-text`, `--stagebook-hover-bg` on the hovered row, `--stagebook-primary` for the checked row's mark, `--stagebook-row-min-height` for row height, and an inset `--stagebook-focus-ring` on the row the keyboard walks to). It also no longer opens the OS's own picker on Android or iOS. The keyboard model, roles and names are still the native `<select>`'s. Engines without it (Firefox, until it ships) ignore the value and keep the native popup, so the trigger is identical on both paths and only the popup differs. `color-scheme: light` is pinned on the `<select>` itself as well as at `:root`, so a host that never loads `stagebook/styles` still gets a light popup where the OS draws one.
