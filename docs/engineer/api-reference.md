...
**Native control themes.** Select, TextArea and Slider pin `color-scheme: light` on their native controls, including when `stagebook/styles` is omitted or a parent advertises a dark scheme. Forced-colors accessibility settings remain available. Select’s disclosure arrow uses `--stagebook-select-chevron` (`#6b7280`), independently of its text and border. The native select still owns keyboard and pointer interaction.

**Color hierarchy.** `--stagebook-text-muted` defaults to `#626977` for secondary labels, hints and counters. Unchecked radio/checkbox outlines use `--stagebook-choice-border` (`#858c99`), independently of the lighter `--stagebook-border` (`#d1d5db`) on fields and secondary buttons. `--stagebook-slider-tick` (`#6b7280`) colors both tick levels: major ticks are opaque and 16px high; minor ticks are 6px high at 0.7 opacity. Timeline ruler timestamps use `--stagebook-timeline-ruler-text` (`#737373`). These specific tokens replace those parts’ previous use of general border, muted-text or decoration tokens; set them explicitly when adapting a host theme. The normal timer fill uses `--stagebook-timer-fill`, defaulting to `--stagebook-primary`. See the [contrast hierarchy decision](../decisions/2026-09-contrast-hierarchy.md) for the approved defaults and remaining accessibility questions.

**Loading indicator.** `Loading` normally spins. With `prefers-reduced-motion: reduce`, it shows a static indicator and the localized loading label. Its accessible name uses the same message in both modes; the visible text is not announced a second time.

**`data-testid` placement.** Single-element controls (`Button`, `Select`) put the caller's `data-testid` on the interactive element itself, so `getByTestId(…)` can be driven directly — Playwright's `selectOption()` requires a real `<select>` and `inputValue()` a form control, so both throw on a wrapper `<div>`. Composite controls (`RadioGroup`, `CheckboxGroup`) put theirs on the group wrapper, since there is no single element to name, and tag inner parts with their own testids (`option`). `Slider` takes no caller testid at all — it exposes fixed ones (`slider` on the wrapper, plus `slider-track`, `slider-thumb`, `slider-value-badge`).

**Icon-only buttons.** `<Button icon aria-label="Refresh devices">` renders a square target with the same variants, tokens, focus halo and disabled treatment as the text button. Its box is a `Select`'s — line-height, padding and border, floored by `--stagebook-row-min-height` — so the two are the same height in a row and stay so when a host raises the token; when using a `label`, align the row to the control below it. `aria-label` is required: an icon has no text to name the button by (WCAG 4.1.2), so omitting it is a type error, and a `console.error` at runtime for JS consumers (an empty or whitespace-only string counts as missing). `title` is forwarded as the native tooltip and may repeat the label; it is never the name — a browser would fall back to it, but touch and screen-reader users don't get it — and disabled buttons retain pointer hit-testing so the browser can show it on hover. Native `disabled` semantics still prevent activation and keyboard focus, while hover/pressed fills apply only to enabled buttons. Mark the glyph `aria-hidden` so the name is the label alone, and draw it in `currentColor` so it inherits the variant's text colour, which clears 3:1 on both fills (WCAG 1.4.11). Both attributes are forwarded verbatim on every `Button`, so a text button with neither prop renders neither; an `aria-label` on a text button must contain its visible text (WCAG 2.5.3).
...
**`Select` picker.** The options list is opted into the customizable select (`appearance: base-select`, #627). On engines that have it — Chrome and Edge 135+, Safari 27 — the picker is a top-layer popover in the page: anchored under the trigger, pinned to its width (long labels wrap into taller rows; the trigger itself stays one line), and painted with the same tokens as the rest of the control (`--stagebook-surface`, `--stagebook-border`, `--stagebook-text`, `--stagebook-hover-bg` on the hovered row, `--stagebook-primary` for the checked row's mark, `--stagebook-row-min-height` for row height, and an inset `--stagebook-focus-ring` on the row the keyboard walks to). The engine may flip it above the trigger when there is no room below (Chromium does; WebKit not dependably). It also no longer opens the OS's own picker on Android or iOS. The keyboard model, roles and names are still the native `<select>`'s: Space or an arrow key opens, arrows walk the rows without changing the value, Enter or a click commits, Escape closes and returns focus. One engine difference worth knowing when a `placeholder` is set: Chromium opens with the first _enabled_ row focused, so Space then Enter commits that row, while WebKit focuses the placeholder row and commits nothing. Engines without it (Firefox, until it ships) ignore the value and keep the native popup, so the trigger is identical on both paths and only the popup differs. `color-scheme: light` is pinned on the `<select>` itself as well as at `:root`, so a host that never loads `stagebook/styles` still gets a light popup where the OS draws one.
